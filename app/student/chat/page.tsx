...
export default function StudentChatPage() {
  ...
  useEffect(() => {
    async function loadData() {
      if (currentUser?.id) {
        const res = await getChatContacts(currentUser.id, 'student');
  ...
  useEffect(() => {
    ...
    if (activeContact) {
      ...
      interval = setInterval(() => fetchMsgs(true), 3000);
    }
  ...
  const handleSend = async () => {
    ...
    if (currentUser?.id) {
      await sendMessage(currentUser.id, activeContact.id, msgContent);
    }
  };
  ...
  return (
    <div className="h-[calc(100vh-120px)] bg-white rounded-2xl shadow-sm border border-slate-200 flex overflow-hidden">
      ...
      <div className="w-80 border-r border-gray-100 flex flex-col bg-slate-50">
        <div className="p-4 border-b border-gray-100">
          <div className="relative">
            ...
            <Input placeholder="搜索咨询师..." className="pl-9 bg-white" />
          </div>
        </div>
          ...
          {loadingContacts ? (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-slate-500">{contact.college || '心理中心'}</div>
              </div>
            </div>
          ))}
        </ScrollArea>
      ...
      <div className="flex-1 flex flex-col bg-white">
        {activeContact ? (
          <>
            <div className="h-16 border-b border-gray-100 flex items-center px-6 font-bold text-slate-800">
              {activeContact.name} 老师
            </div>
